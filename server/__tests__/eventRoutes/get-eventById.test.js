...
describe("GET /events/:eventId", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.log(user1_token, "ini token");
  ...
  test("Test GET EVENT", async () => {
    const response = await request(app).get(`/event/${event1_Id}`).set('Authorization', `Bearer ${user1_token}` )
    ...
    expect(response.body).toHaveProperty('player',  eventsData[0].player)
