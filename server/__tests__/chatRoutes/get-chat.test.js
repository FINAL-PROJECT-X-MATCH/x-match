...
describe("get /chats/:eventId", () => {
    ...
    beforeAll(async () => {
        try {
            ...
            const seedChat = await chatCollection.insertMany(chatData)
        } catch (err) {
            console.error("Error connecting to the database:", err);
        }
    });

    test("Test get chat", async () => {
        const response = await request(app)
            .get(`/chats/${event1_Id.toString()}`)
            .set('Authorization', `Bearer ${user1_token}`)
        console.log(response.body[0], "ini hasil ");
        expect(response.status).toBe(200)
        expect(response.body).toBeInstanceOf(Array)
        expect(response.body[0]).toBeInstanceOf(Object)
        expect(response.body[0]).toHaveProperty('eventId', expect.any(String))
