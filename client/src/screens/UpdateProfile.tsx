...
const UpdateProfile: React.FC = () => {
  ...
  const [isPremium, setIsPremium] = useState<boolean>(user?.member === 'premium');
  ...
  const fetchUserData = async () => {
    try {
      ...
      setIsPremium(userData.member === 'premium');
      setLoading(false);
    } catch (error) {
  ...
  const handleUpgradeToPremium = async () => {
    try {
      ...
      Alert.alert('Error', 'There was an error upgrading your account to premium.');
      setLoading(false);
    }
  };
  ...
  const handleWebViewNavigationStateChange = async (navState: any) => {
    if (navState.url.includes('status_code=200')) {
      ...
      try {
        ...
        updateUser({ member: 'premium' });
        setIsPremium(true);
        Alert.alert('Success', 'Your account has been upgraded to premium!');
      } catch (error) {
        console.error('Error updating user to premium:', error);
        Alert.alert('Error', 'There was an error updating your account to premium.');
      }
    }
  };
      ...
      {showWebView ? (
        ...
        <ScrollView contentContainerStyle={tw`flex-1 justify-center items-center p-5`}>
          ...
          <View style={tw`w-full bg-white p-5 rounded-3xl shadow-lg`}>
            ...
            {!isPremium && (
              <TouchableOpacity style={[tw`p-4 rounded-full mb-4 flex-row justify-center items-center`, { backgroundColor: 'rgb(54 162 235)' }]} onPress={handleUpgradeToPremium}>
                {loading ? (
                  <ActivityIndicator size="small" color="white" />
                ) : (
                  <>
                    <Ionicons name="card" size={24} color="white" style={tw`mr-2`} />
                    <Text style={tw`text-white text-center text-lg`}>Upgrade to Premium</Text>
                  </>
                )}
              </TouchableOpacity>
            )}
